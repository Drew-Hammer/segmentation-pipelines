...
# Step 4: Generate utterance arrays JSON
echo "--- Generating utterance arrays JSON... ---"
./utteranceArrays segmented_with_ids.txt utterance_arrays.json
echo "--- Cleaning Up... ---"
find . -type f ! -name 'utterance_arrays.json' ! -name 'TRANS.txt' \( -name '*.txt' -o -name 'textIsolator' -o -name 'prependUtteranceID' -o -name 'utteranceArrays' -o -name '*.utf8' \) -exec rm -f {} +
echo " "
